<script setup>
import { ref, onMounted, computed } from "vue"
const userAgent = ref("")

const props = defineProps({
  msg: String,
  facebook: Boolean,
  twitter: Boolean,
  whatsapp: Boolean,
  messenger: Boolean
})

// you can use personalized content in some shares. For example WhatsApp and Twitter
const personalMessage = ref("Frituur De Puntzak")


onMounted(() => {
    
    userAgent.value = androidOrIOS()
    
})


function androidOrIOS() {
    const userAgent = navigator.userAgent;
    if(/android/i.test(userAgent)){
        return 'android';
    }
    else if(/iPad|iPhone|iPod/i.test(userAgent)){
        return 'ios';
    }
    else {
        return 'other'
    }
}


const cptFbShare = computed(() => {
    return "https://www.facebook.com/sharer/sharer.php?u=https://www.nieuwsblad.be/beste-frituur-van-vlaanderen?SOURCE=fb_share" + (userAgent.value != "ios" ? '&navigation=external' : ''); 
})


const cptWaShare = computed(() => {
    return "https://wa.me/" + (userAgent.value != "ios" ? '?navigation=external' : '?')  + "&text=" + personalMessage.value + " is voor mij de Beste Frituur van Vlaanderen. Breng ook je stem uit via de Nieuwsblad-app en win een jaar lang gratis frieten. Meer info: www.nieuwsblad.be/frituur" 
})

const cptTwShare = computed(() => {
    return "https://twitter.com/intent/tweet?url=https://www.nieuwsblad.be/beste-frituur-van-vlaanderen&SOURCE=tw_share&text=" + personalMessage.value + " is voor mij de Beste Frituur van Vlaanderen. Breng ook je stem uit via de Nieuwsblad-app en win een jaar lang gratis frieten." + (userAgent.value != "ios" ? '&navigation=external' : ''); 
})

const cptMessShare = computed(() => {
    return "fb-messenger://share/?link=https://www.nieuwsblad.be/beste-frituur-van-vlaanderen?SOURCE=mess_share" + (userAgent.value != "ios" ? '&navigation=external' : ''); 
})



</script>


<template>

    

    <div class="my-0 text-center" :class="androidOrIOS()">

        <h3 class="text-2xl brand-font-primary mb-3">{{ msg }}</h3>


        <a v-if="facebook" :href="cptFbShare"
            target="_blank" class="share-btn-fb share-btn  text-white rounded-xl no-underline  m-2 d-inline-block ">
            <svg width="24" height="24" class="mr-2"
                viewBox="0 0 24 24">
                <path fill="currentColor"
                    d="M12 2.04C6.5 2.04 2 6.53 2 12.06C2 17.06 5.66 21.21 10.44 21.96V14.96H7.9V12.06H10.44V9.85C10.44 7.34 11.93 5.96 14.22 5.96C15.31 5.96 16.45 6.15 16.45 6.15V8.62H15.19C13.95 8.62 13.56 9.39 13.56 10.18V12.06H16.34L15.89 14.96H13.56V21.96A10 10 0 0 0 22 12.06C22 6.53 17.5 2.04 12 2.04Z">
                </path>
            </svg>
            <span >Deel op Facebook</span>
        </a>

        
        <a v-if="whatsapp" :href="cptWaShare"
            target="_blank" class="share-btn-wa share-btn  text-white rounded-xl no-underline m-2 d-inline-block ">
            <svg width="24" height="24" class="mr-2"
                viewBox="0 0 24 24">
                <path fill="currentColor"
                    d="M12.04 2C6.58 2 2.13 6.45 2.13 11.91C2.13 13.66 2.59 15.36 3.45 16.86L2.05 22L7.3 20.62C8.75 21.41 10.38 21.83 12.04 21.83C17.5 21.83 21.95 17.38 21.95 11.92C21.95 9.27 20.92 6.78 19.05 4.91C17.18 3.03 14.69 2 12.04 2M12.05 3.67C14.25 3.67 16.31 4.53 17.87 6.09C19.42 7.65 20.28 9.72 20.28 11.92C20.28 16.46 16.58 20.15 12.04 20.15C10.56 20.15 9.11 19.76 7.85 19L7.55 18.83L4.43 19.65L5.26 16.61L5.06 16.29C4.24 15 3.8 13.47 3.8 11.91C3.81 7.37 7.5 3.67 12.05 3.67M8.53 7.33C8.37 7.33 8.1 7.39 7.87 7.64C7.65 7.89 7 8.5 7 9.71C7 10.93 7.89 12.1 8 12.27C8.14 12.44 9.76 14.94 12.25 16C12.84 16.27 13.3 16.42 13.66 16.53C14.25 16.72 14.79 16.69 15.22 16.63C15.7 16.56 16.68 16.03 16.89 15.45C17.1 14.87 17.1 14.38 17.04 14.27C16.97 14.17 16.81 14.11 16.56 14C16.31 13.86 15.09 13.26 14.87 13.18C14.64 13.1 14.5 13.06 14.31 13.3C14.15 13.55 13.67 14.11 13.53 14.27C13.38 14.44 13.24 14.46 13 14.34C12.74 14.21 11.94 13.95 11 13.11C10.26 12.45 9.77 11.64 9.62 11.39C9.5 11.15 9.61 11 9.73 10.89C9.84 10.78 10 10.6 10.1 10.45C10.23 10.31 10.27 10.2 10.35 10.04C10.43 9.87 10.39 9.73 10.33 9.61C10.27 9.5 9.77 8.26 9.56 7.77C9.36 7.29 9.16 7.35 9 7.34C8.86 7.34 8.7 7.33 8.53 7.33Z">
                </path>
            </svg>
            <span >Deel op WhatsApp</span>
        </a>

        <a v-if="messenger " :href="cptMessShare"
            target="_blank" class="share-btn-messenger share-btn  text-white rounded-xl no-underline m-2 d-inline-block ">
            <svg width="24" height="24" class="mr-2"
                viewBox="0 0 24 24">
                <path fill="currentColor"
                    d="M12,2C6.36,2 2,6.13 2,11.7C2,14.61 3.19,17.14 5.14,18.87C5.3,19 5.4,19.22 5.41,19.44L5.46,21.22C5.5,21.79 6.07,22.16 6.59,21.93L8.57,21.06C8.74,21 8.93,20.97 9.1,21C10,21.27 11,21.4 12,21.4C17.64,21.4 22,17.27 22,11.7C22,6.13 17.64,2 12,2M18,9.46L15.07,14.13C14.6,14.86 13.6,15.05 12.9,14.5L10.56,12.77C10.35,12.61 10.05,12.61 9.84,12.77L6.68,15.17C6.26,15.5 5.71,15 6,14.54L8.93,9.87C9.4,9.14 10.4,8.95 11.1,9.47L13.44,11.23C13.66,11.39 13.95,11.39 14.16,11.23L17.32,8.83C17.74,8.5 18.29,9 18,9.46Z">
                </path>
            </svg>
            <span >Deel op Messenger</span>
        </a>


        <a v-if="twitter" :href="cptTwShare"
            target="_blank" class="share-btn-tw share-btn  text-white rounded-xl no-underline m-2 d-inline-block">
            <svg width="24" height="24" class="mr-2"
                viewBox="0 0 24 24">
                <path fill="currentColor"
                    d="M22.46,6C21.69,6.35 20.86,6.58 20,6.69C20.88,6.16 21.56,5.32 21.88,4.31C21.05,4.81 20.13,5.16 19.16,5.36C18.37,4.5 17.26,4 16,4C13.65,4 11.73,5.92 11.73,8.29C11.73,8.63 11.77,8.96 11.84,9.27C8.28,9.09 5.11,7.38 3,4.79C2.63,5.42 2.42,6.16 2.42,6.94C2.42,8.43 3.17,9.75 4.33,10.5C3.62,10.5 2.96,10.3 2.38,10C2.38,10 2.38,10 2.38,10.03C2.38,12.11 3.86,13.85 5.82,14.24C5.46,14.34 5.08,14.39 4.69,14.39C4.42,14.39 4.15,14.36 3.89,14.31C4.43,16 6,17.26 7.89,17.29C6.43,18.45 4.58,19.13 2.56,19.13C2.22,19.13 1.88,19.11 1.54,19.07C3.44,20.29 5.7,21 8.12,21C16,21 20.33,14.46 20.33,8.79C20.33,8.6 20.33,8.42 20.32,8.23C21.16,7.63 21.88,6.87 22.46,6Z">
                </path>
            </svg>
            <span >Deel op Twitter</span>
        </a>


        

    </div>
</template>

<style scoped>
.ios {display:none}
.share-btn-fb {background:#1877F2 !important}
.share-btn-wa {background:#25d366 !important}
.share-btn-messenger {background:#1877F2 !important}
.share-btn-tw {background:#1d9bf0 !important}

.share-btn {font-size:0.9rem; padding:0.8rem;}
</style>